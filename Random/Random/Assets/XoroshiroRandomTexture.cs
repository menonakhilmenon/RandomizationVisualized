...
public class XoroshiroRandomTexture : MonoBehaviour
{
    ...
    {
        int.TryParse(itField.text, out iterations);
        t0 = System.DateTime.Now;
        th = new Thread(ApplyBarnsley);
        th.Start();
    }
    private void Update()
    {
        if (th != null)
        {
            ...
            {
                th = null;
                delayText.text = "Delay : " + (System.DateTime.Now - t0).TotalMilliseconds.ToString() + "ms";
                ApplyColor();
            }
        }
    }
